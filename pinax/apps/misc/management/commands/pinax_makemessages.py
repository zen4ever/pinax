...
def make_messages(localedir, appdir, locale=None, domain='django', verbosity='1', all=False, extensions=None):
    """
    Uses the locale directory from the Django SVN tree or an application/
    project to process all
    """
    os.chdir(os.path.abspath(os.path.join(appdir, '..')))
...
class Command(AppCommand):
    ...
    def handle(self, *app_labels, **options):
        ...
        if os.path.isdir('locale'):
            self.localedir = settings.PROJECT_ROOT
        else:
            raise CommandError("This script should be run from your project tree. If you did indeed run it from a fresh checkout of your project, maybe you are just missing the conf/locale (in the django tree) or locale (for project and application) directory? It is not created automatically, you have to create it by hand if you want to enable i18n for your project.")
        if not app_labels:
    ...
    def handle_app(self, app, *args, **options):
        ...
        make_messages(self.localedir, appdir, locale, domain, verbosity, process_all, extensions)
